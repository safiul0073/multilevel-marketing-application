...
const labels = [
    ...
    { key: "created_at", label: "Transaction At" },
];
...
const Transaction = () => {
    ...
    const { data: excelData } = getTransactions({
        from_date: fromDate,
        to_date: toDate,
        search: useDebounce(searchKeyword),
        type: type,
    ...
    const [tableLabels, setLabels] = React.useState(labels);
    ...
    const mappedObj = (d) => {
        if (type === "all") {
            return {
                ...
                type: getTypeStyle(d?.type).name,
                member: d?.member?.username,
                amount: d?.amount,
                status: d?.status ? "Confirmed" : "Pending",
            };
        }
        return {
            ...d,
    ...
    const changeType = (n) => {
        if (n === "all") {
            tableLabels.splice(2, 2);
            setLabels(() => tableLabels);
        }
        setType(n.type);
        setTypes(() =>
            transactionType.map((c) => {
                if (n.type === c.type) {
                    c.is_active = true;
                } else {
                    c.is_active = false;
            ...
            <Card
                headerSlot={
                    <SearchBar
                        ...
                        labels={labels}
                        reportName="Transaction_list_report"
                ...
                <div className="sm:flex sm:items-center">
                    <div className="md:flex gap-3">
                        {types.map((n) => (
                            <button
                                key={Math.random()}
                                onClick={() => changeType(n)}
                                className={
                                    "px-2 my-2 py-1 rounded-md mx-2 " +
                                    (n.is_active ? n.selected_style : n.style)
                                }
                            >
                                {n.name}
                            </button>
